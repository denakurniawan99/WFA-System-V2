...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 9. Manajemen Zoom Meeting
  // Memakai link & passcode Zoom perusahaan dari Pengaturan WFA (link asli yang bisa dibuka).
  const createZoomMeeting = (data: {
    ...
    const zoomLink = (wfaSettings.companyZoomLink || '').trim();
    if (!zoomLink) {
      showToast('Link Zoom perusahaan belum diatur. Minta HRD mengisinya di menu Pengaturan WFA.', 'warning');
      return null;
    }

    // Ambil Meeting ID dari link (…/j/85620165861?pwd=…) lalu format 3-4-4 / 3-3-4
    const digits = (zoomLink.match(/\/j\/(\d+)/)?.[1]) || '';
    const formattedMeetingId =
      digits.length === 11
    ...
    // Otomatis buat notifikasi Zoom untuk seluruh tim
    const now = new Date();
    const timeStr = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')} WIB`;
    const newZoomWarn: WarningItem = {
      id: 'warn-zoom-' + Date.now(),
      senderId: currentUser.id,
      senderName: currentUser.name,
      recipientId: 'all',
      recipientName: 'Seluruh Tim Karyawan',
      type: 'zoom_pagi',
      title: data.isUrgent ? `🚨 Zoom Urgen: ${data.title}` : `Undangan Zoom: ${data.title}`,
      message: `Koordinator mengundang Anda ke Zoom koordinasi "${data.title}" (${data.time} WIB). Klik untuk langsung bergabung (wajib absen pagi terlebih dahulu).`,
      date: data.date,
      createdAt: timeStr,
      status: 'terkirim',
    };
    setWarnings((prev) => [newZoomWarn, ...prev]);

    if (data.isUrgent) {
      showToast(`🚨 Zoom Urgen "${data.title}" berhasil dibuat & disebarkan ke tim!`, 'warning');
    } else {
      showToast(`⚡ Jadwal Zoom "${data.title}" dibuat & dibagikan ke seluruh tim`, 'success');
    }
