...
export const PerformaTimView: React.FC = () => {
  ...
  const allDivisions = useMemo(
    () =>
      Array.from(
        new Set(
          allUsers.filter((u) => u.role === 'karyawan' && u.leaderId === currentUser.id).map((u) => u.division)
        )
      ),
    [allUsers, currentUser.id]
  );

  // Filter Divisi: koordinator bisa fokus melihat performa satu divisi tertentu lebih spesifik
  ...
  const teamMembers = useMemo(
    ...
    [allUsers, currentUser.id, filterDivision]
  );
  const myTeamIds = useMemo(() => new Set(teamMembers.map((m) => m.id)), [teamMembers]);
  const scopedEntries = useMemo(
    () => entries.filter((e) => myTeamIds.has(e.userId)),
    [entries, myTeamIds]
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#004080] to-[#0060b5] rounded-2xl p-6 text-white shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="px-5 py-3 rounded-xl bg-white/10 border border-white/20 text-center shrink-0">
          ...
          <div className="text-[10px] text-sky-300 mt-0.5 font-medium">
            {filterDivision === 'all' ? 'Berdasarkan Reviu Leader' : `Divisi ${filterDivision}`}
          </div>
        </div>
      </div>

      {/* Filter Divisi: fokus analisis ke satu divisi tertentu */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-4 flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <p className="text-[11px] text-slate-400 sm:ml-auto">
          Grafik, KPI, dan peringkat di bawah otomatis menyesuaikan dengan divisi yang dipilih.
        </p>
      </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Area
                    ...
                    fill="url(#colorTodo)"
                  >
                    <LabelList dataKey="Skor To-Do Centang (%)" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#1d4ed8' }} />
                  </Area>
                  <Area
                    type="monotone"
                  ...
                  >
                    <LabelList dataKey="Nilai Leader" position="bottom" style={{ fontSize: 10, fontWeight: 700, fill: '#047857' }} />
                  </Area>
                  <Area
                    type="monotone"
                    dataKey="Komunikasi"
                  ...
                  >
                    <LabelList dataKey="Komunikasi" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#6d28d9' }} />
                  </Area>
                </AreaChart>
              </ResponsiveContainer>
            ) : (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={memberBarData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="Total Target Task" fill="#cbd5e1" radius={[4, 4, 0, 0]} maxBarSize={40}>
                <LabelList dataKey="Total Target Task" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#64748b' }} />
              </Bar>
              <Bar dataKey="Task Dicentang" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={40}>
                <LabelList dataKey="Task Dicentang" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#1d4ed8' }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
