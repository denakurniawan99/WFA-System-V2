...
export const AnalisisPerformaHrdView: React.FC = () => {
  ...
  const scopedEntries = useMemo(
    () => (filterDivision === 'all' ? entries : entries.filter((e) => e.division === filterDivision)),
    [entries, filterDivision]
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-4 flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <p className="text-[11px] text-slate-400 sm:ml-auto">
          Seluruh kartu, grafik, dan daftar di bawah otomatis menyesuaikan dengan divisi yang dipilih.
        </p>
      </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={divisionBarData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="Rata-rata Centang To-Do (%)" fill="#0ea5e9" radius={[4, 4, 0, 0]} maxBarSize={40}>
                  <LabelList dataKey="Rata-rata Centang To-Do (%)" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#0369a1' }} />
                </Bar>
                <Bar dataKey="Rata-rata Reviu Leader" fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={40}>
                  <LabelList dataKey="Rata-rata Reviu Leader" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#1d4ed8' }} />
                </Bar>
                <Bar dataKey="Rata-rata Komunikasi" fill="#8b5cf6" radius={[4, 4, 0, 0]} maxBarSize={40}>
                  <LabelList dataKey="Rata-rata Komunikasi" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#6d28d9' }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          {companyTrendData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={companyTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                >
                  <LabelList dataKey="Rata-rata Perusahaan (%)" position="top" style={{ fontSize: 10, fontWeight: 700, fill: '#0369a1' }} />
                </Area>
                <Area
                  type="monotone"
                ...
                >
                  <LabelList dataKey="Evaluasi Leader" position="bottom" style={{ fontSize: 10, fontWeight: 700, fill: '#047857' }} />
                </Area>
                <Area
                  type="monotone"
                  dataKey="Komunikasi"
