...
export const NotifikasiView: React.FC<NotifikasiViewProps> = ({ onNavigate }) => {
  const { currentUser, warnings, zoomMeetings, entries, selectedDate, showToast } = useApp();
  ...
  const notifications: AppNotification[] = useMemo(() => {
    ...
    // 3. Notifikasi Zoom Meeting
    zoomMeetings.forEach((z) => {
      list.push({
        id: `zoom-${z.id}`,
        type: 'zoom',
        title: z.isUrgent ? `🚨 Zoom Urgen: ${z.title}` : `Undangan Zoom: ${z.title}`,
      ...
      <div className="bg-gradient-to-r from-[#004080] to-[#0060b5] rounded-2xl p-6 text-white shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-sky-100/90 mt-1 max-w-xl">
            Pantau seluruh pengingat presensi harian, evaluasi review leader, panggilan Zoom, dan surat teguran secara terintegrasi.
          </p>
        </div>
