...
export const PerformaTimView: React.FC = () => {
  ...
  const teamMembers = useMemo(() => allUsers.filter((u) => u.role === 'karyawan'), [allUsers]);

  // Agregat performa untuk masing-masing karyawan
  const memberStats = useMemo(() => {
    const stats = teamMembers.map((member) => {
      const userEntries = entries.filter((e) => e.userId === member.id);
  ...
  const trendData = useMemo(() => {
    const datesMap: { [date: string]: { todoSum: number; leaderSum: number; count: number; leaderCount: number } } = {};

    entries.forEach((e) => {
      if (!datesMap[e.date]) {
        datesMap[e.date] = { todoSum: 0, leaderSum: 0, count: 0, leaderCount: 0 };
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#004080] to-[#0060b5] rounded-2xl p-6 text-white shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="px-5 py-3 rounded-xl bg-white/10 border border-white/20 text-center shrink-0">
          ...
          <div className="text-[10px] text-sky-300 mt-0.5 font-medium">Berdasarkan Reviu Leader</div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Perbandingan antara Skor Centang To-Do Mandiri vs Nilai Evaluasi Leader harian
              </p>
            </div>
